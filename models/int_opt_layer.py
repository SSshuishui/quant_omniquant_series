...
class QuantOPTAttention(nn.Module):
    ...
    def __init__(
        ...
        self.k_proj = QuantLinear(
            ...
            None if attention_precision is None else attention_precision["self_attn.k_proj"],
        )
        self.v_proj = QuantLinear(
            org_module.v_proj,
            args.weight_quant_params,
            args.act_quant_params,
            None if attention_precision is None else attention_precision["self_attn.v_proj"],
        )
        self.q_proj = QuantLinear(
            org_module.q_proj,
            args.weight_quant_params,
            args.act_quant_params,
            None if attention_precision is None else attention_precision["self_attn.q_proj"],
        )
        self.out_proj = QuantLinear(
            org_module.out_proj, 
            args.weight_quant_params, 
            args.act_quant_params, 
            None if attention_precision is None else attention_precision["self_attn.out_proj"],
        )
...
class OmniQuantOPTDecoderLayer(nn.Module):
    def __init__(
        self,
        config,
        ori_layer,
        args,
        decoderlayer_precision=None
    ):
        super().__init__()
    ...
    def smooth_and_quant_inplace(self):
        from models.omniquant_transformation import smooth_ln_fcs_inplace, smooth_fc_fc_inplace, smooth_q_k_inplace, truncate_number
    ...
    def smooth_and_quant_temporary(self):
        from models.omniquant_transformation import smooth_ln_fcs_temporary, smooth_fc_fc_temporary, smooth_q_k_temporary, truncate_number
...
class AffineQuantOPTDecoderLayer(nn.Module):
    def __init__(
        self,
        config,
        ori_layer,
...
class LRQuantOPTDecoderLayer(nn.Module):
    def __init__(
        self,
        config,
        ori_layer,
        ...
        super().__init__()
        from quantize.lrq_norm import RLQLayerNorm
        self.embed_dim = config.hidden_size
        self.self_attn = QuantOPTAttention(
            org_module=ori_layer.self_attn,
            embed_dim=self.embed_dim,
...
class RPTQQuantOPTDecoderLayer(nn.Module):
    def __init__(
        self,
        config,
        ori_layer,
